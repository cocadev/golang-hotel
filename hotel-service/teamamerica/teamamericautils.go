...
const (
	TeamAmericaHotelPrefix string = "restel_"
)
...
func GetCalculatedPrice(cancelPolicy *CancelPolicy, nightlyPrice float32, total float32) float32 {
	if cancelPolicy.PenaltyType == "Nights" {
		return cancelPolicy.PenaltyAmount * nightlyPrice
	} else if cancelPolicy.PenaltyType == "Percent" {
		return cancelPolicy.PenaltyAmount * total / 100.0
	} else if cancelPolicy.PenaltyType == "Dollars" {
		return cancelPolicy.PenaltyAmount
	}
